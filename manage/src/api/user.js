// import request from '@/utils/request'

export function login(params) {
  // return request({
  //   url: '/user/login',
  //   method: 'get',
  //   params
  // })
  return new Promise(res => res({ 'code': 20000, 'data': { 'token': 'admin-token' }}))
}

export function getInfo(token) {
  // return request({
  //   url: '/user/info',
  //   method: 'get',
  //   params: { token }
  // })
  return new Promise(res => res({ 'code': 20000, 'data': { 'roles': ['admin'], 'introduction': 'I am a super administrator', 'avatar': 'data:image/jpeg;base64,', 'name': 'Super Admin' }}))
}

export function logout() {
  // return request({
  //   url: '/user/logout',
  //   method: 'get'
  // })
  return new Promise(res => res({ 'code': 20000, 'data': 'success' }))
}
